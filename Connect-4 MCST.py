...
class GameState:
    ...
    '''
    favors letting you block its winning spots, as long as it will eventually win
    since rollout policy is more streamlined, it goes farther into the future with those forcing moves
    '''
...
class MCTSNode:
    ...
    # def is_fully_expanded_tree(self):
    #     if not self.is_fully_expanded():
    #         return False
    #     return all(child.is_fully_expanded_tree() for child in self.children)

def rollout_multi(state_and_turn):
    state, turn = state_and_turn
    curr_state = state.copy()
    curr_turn = turn
    turns_ahead = 0
    while not curr_state.is_terminal():
        slots = curr_state.rollout_policy()
        curr_state = curr_state.invert().place_piece(slots)
        curr_turn *= -1
        turns_ahead += 1
    
    if curr_state.get_winner():
        return 1 if curr_turn == turn else -1 
    return 0 


import multiprocessing
def run_parallel_rollouts(state, turn, num_rollouts):
    args = [(state, turn)] * num_rollouts
    with multiprocessing.Pool() as pool:
        return pool.map(rollout_multi, args)
...
def mcts(root_state, rollouts = 1, itterations = 1_000, run_time = 0):
    ...
    # book_move = openning_book(root_state)
    # if book_move:
    #     return book_move
    ...
    while i < itterations or (time.time() - start) < run_time:
        ...
        # if parallel and rollouts > 1:
            # args = [(curr_node.state.copy(), curr_node.turn)] * rollouts
            # results = pool.map(rollout_multi, args)
            # winner = np.mean(results)
        #else:
    ...
    return root_node.best_move().state.last_slot


def openning_book(state):
    return False

parallel = False #multiprocessing BROKEN when computer knows it will lose?
